...
plot_belowLOD_per_analyte <- function(input, output, session, values) {
  ...
  output$plot_belowLOD_per_analyte <- renderPlot({
    ...
    lapply(values$upload_data, function(x){
      ...
      geom_point(aes(color = number_of_sample_below_lod), alpha = 0.5, size = 6, shape = 15)+
      scale_color_gradient(low = "grey", high = "blue")+
      geom_text(aes(label = ifelse(number_of_sample_below_lod == 0, NA, number_of_sample_below_lod)))+
